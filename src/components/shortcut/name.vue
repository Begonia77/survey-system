<template>
  <div class="qs-layout">
    <div class="sr-show" @mouseenter="changePop" @mouseleave="popup = false">
      <div>
        <div class="sr-title">
          <span>{{ qsIndex }}.&nbsp;{{ nameForm.title }}</span>
        </div>
        <el-input prefix-icon="el-icon-user" v-model="ans"> </el-input>
      </div>
...
export default {
  ...
  created() {
    if (this.qsModel === "true") {
      ...
      this.nameForm.qs_id = question.qs_id;
    }

    if (this.qsEdit === "true") {
      this.show = false;
      if (this.qsData) {
        this.nameForm.qs_id = this.qsData.qs_id;
      } else {
        this.show = true;
      }
    }
